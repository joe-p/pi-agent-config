...
function dirContainsNestedProjectFile(
  dir: string,
  dirSuffix: string,
  markerFile: string,
): boolean {
...
function findRootSwift(file: string, cwd: string): string | undefined {
  ...
  while (current.length >= stop.length) {
    ...
    if (dirContainsNestedProjectFile(current, ".xcodeproj", "project.pbxproj"))
      return current;
    if (
      dirContainsNestedProjectFile(
        current,
        ".xcworkspace",
...
async function ensureJetBrainsKotlinLspInstalled(): Promise<
  string | undefined
> {
  ...
  try {
    ...
    const okDownload = await runCommand(
      curl,
      ["-L", "-o", zipPath, url],
      installDir,
    );
    ...
    const okUnzip = await runCommand(
      unzip,
      ["-o", zipPath, "-d", installDir],
      installDir,
    );
    try {
...
async function spawnSourcekitLsp(
  ...
  return spawnWithFallback(
    xcrun,
    [["sourcekit-lsp"], ["sourcekit-lsp", "--stdio"]],
    root,
  );
}
...
export const LSP_SERVERS: LSPServerConfig[] = [
  {
    ...
    extensions: [".dart"],
    findRoot: (f, cwd) =>
      findRoot(f, cwd, ["pubspec.yaml", "analysis_options.yaml"]),
    spawn: async (root) => {
      let dart = which("dart");
  ...
  },
  {
    ...
    findRoot: (f, cwd) => {
      if (findNearestFile(path.dirname(f), ["deno.json", "deno.jsonc"], cwd))
        return undefined;
      return findRoot(f, cwd, [
        "package.json",
        "tsconfig.json",
        "jsconfig.json",
      ]);
  ...
  },
  {
    ...
    extensions: [".vue"],
    findRoot: (f, cwd) =>
      findRoot(f, cwd, ["package.json", "vite.config.ts", "vite.config.js"]),
  ...
  },
  {
    ...
    extensions: [".svelte"],
    findRoot: (f, cwd) =>
      findRoot(f, cwd, ["package.json", "svelte.config.js"]),
  ...
  },
  {
    ...
    extensions: [".go"],
    findRoot: (f, cwd) =>
      findRoot(f, cwd, ["go.work"]) || findRoot(f, cwd, ["go.mod"]),
...
export class LSPManager {
    ...
    try {
      ...
      conn.onNotification(
        ...
        (params: { uri: string; diagnostics: Diagnostic[] }) => {
          ...
          const listeners2 =
            fp !== fpRaw ? client.listeners.get(fpRaw) : undefined;
  ...
  private explainNoLsp(absPath: string): string {
    ...
    if (ext === ".kt" || ext === ".kts") {
      ...
      if (this.broken.has(k))
        return `Kotlin LSP failed to initialize for root: ${root}`;
    ...
    if (ext === ".swift") {
      ...
      if (this.broken.has(k))
        return `sourcekit-lsp failed to initialize for root: ${root}`;
  ...
  private async pullDiagnostics(
    ...
    if (
      !client.capabilities ||
      !(client.capabilities as any).diagnosticProvider
    ) {
    ...
    try {
      const res: any = await client.connection.sendRequest(
    ...
    try {
      ...
      if (match?.kind === DocumentDiagnosticReportKind.Full) {
        return {
          diagnostics: Array.isArray(match.items) ? match.items : [],
          responded: true,
        };
      }
