...
function isContextLike(value: unknown): value is { cwd: string } {
  return (
    !!value &&
    typeof value === "object" &&
    typeof (value as any).cwd === "string"
  );
}
...
function normalizeExecuteArgs(
  ...
  if (isContextLike(signalArg)) {
    return {
      ...
      onUpdate:
        typeof ctxArg === "function"
          ? (ctxArg as ExecuteArgs["onUpdate"])
          : undefined,
      ctx: signalArg,
    };
  }
    ...
    return {
      ...
      onUpdate:
        typeof onUpdateArg === "function"
          ? (onUpdateArg as ExecuteArgs["onUpdate"])
          : undefined,
      ctx: ctxArg,
    };
  }
...
Use bash to find files: find src -name "*.ts" -type f`,
    ...
    async execute(_toolCallId, params, signalArg, onUpdateArg, ctxArg) {
      const { signal, onUpdate, ctx } = normalizeExecuteArgs(
        onUpdateArg,
        ctxArg,
        signalArg,
      );
