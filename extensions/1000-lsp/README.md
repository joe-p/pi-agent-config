...
Forked from MIT licensed https://github.com/prateekmedia/pi-hooks/tree/61b92fa22b4ad237641a5dfb184d92e7c65885fa/lsp
...
1. Tracks files touched by `write`/`edit`
1. Default (`agent_end`): at agent end, sends touched files to LSP and posts a diagnostics message
1. Optional (`edit_write`): per `write`/`edit`, appends diagnostics to the tool result
1. Shows notification with diagnostic summary
1. **Memory Management**: Keeps up to 30 files open per LSP server (LRU eviction), automatically closes idle files (> 60s), and shuts down all LSP servers after 2 minutes of post-agent inactivity (servers restart lazily when files are read again).
1. **Robustness**: Reuses cached diagnostics if a server doesn't re-publish them for unchanged files, avoiding false timeouts on re-analysis.
