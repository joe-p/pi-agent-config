...
export async function readFile(filePath: string): Promise<{ bom: boolean; text: string }> {
  const data = await fs.readFile(filePath)
  const text = new TextDecoder("utf-8", { ignoreBOM: true }).decode(data)
  return split(text)
}

export async function writeWithDirs(filePath: string, content: string): Promise<void> {
